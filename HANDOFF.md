...
- **G3 (진행 중)**: E2E 확대
  ...
  - ⬜ 카드 배틀 / 보스 레이드 미커버. (이번 세션 진입 경로 분석 완료 — 아래 레시피 참고)

  **⚠️ 검증 결과 (2026-06-05 세션): 배틀도 UI 경로로 도달 불가 → test-only 훅 필요.**
  - **좋은 소식**: Phaser canvas는 **헤드리스 크로미움에서 정상 로드됨**(메타버스 패널 렌더 확인). Phaser는 막힘 아님.
  - **막힌 원인**: `getLocalPlayer()`는 `studentName`이 있으면 nickname을 채운 player를 반환(`lib/game-state.ts:856`)
    → StudentLobby 마운트 effect가 `joinedPlayer`를 즉시 설정 → **로비 입장 화면(단원 선택) 건너뜀**.
    `isSimulatedLobby`는 입장 화면의 단원 버튼(`handleInitSimulatedLobby`)으로만 true가 되는데 그 화면이 안 뜸
    → "배틀 스타디움 즉시입장" 버튼은 `(classroomSession?.battleMode || isSimulatedLobby)` 조건이라 **복귀 학생 플로우에선 미노출**.
    CardBattleArena는 UI상 `classroomSession.battleMode`(교사 주도)로만 진입 가능.
  - **결론**: 배틀·레이드 **둘 다 test-only 진입 훅이 정답**. `app/page.tsx`에서 `?e2e=battle` / `?e2e=raid`
    쿼리로 초기 `activeScreen`(+raid는 `player`) 주입이 가장 깔끔. 진입 후 안정 assertion:
    · 배틀 매치메이킹 화면 텍스트 = **`"배틀 상대 찾는 중..."`** (`MatchmakingScreen.tsx:20`)
    · raid 화면 텍스트는 `BossRaidScreen` 1회 읽기로 확정 필요.
  - 시도했던 UI 레시피(아래)는 `studentName` 미설정으로 입장 화면을 띄우면 부분 동작하나, 그 경우 다른 플로우라 불안정.
