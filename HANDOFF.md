...
## 현재 상태 한 줄 요약
**Phase G3 완료.** 전부 그린(tsc·단위30·빌드·E2E17). 다음은 H3/I2/H1.
...
- ✅ **G3 완료**: E2E 17개 (퀴즈 플로우 4개 + 배틀/레이드 2개)
  - `?e2e=battle` → `activeScreen='battle'` 직접 주입, assertion: "배틀 상대 찾는 중..."
  - `?e2e=raid` → `activeScreen='raid'` + `player` 주입, assertion: "BOSS HP:"
  - 훅 위치: `app/page.tsx` mount-only `useEffect` (eslint-disable-next-line react-hooks/exhaustive-deps)
