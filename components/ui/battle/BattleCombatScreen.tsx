      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass-panel p-4 border-cyan-500/20 bg-black/60 relative flex flex-col justify-between min-h-[160px]">
          <div className="flex items-center gap-3">
            <RenderAvatarPreview baseAvatar={studentAvatar} outfit={equippedCosmetics.outfit} expression={equippedCosmetics.expression} accessory={equippedCosmetics.accessory} mount={equippedCosmetics.mount} size="md" />
            <div>
          ...
          <div className="mt-4">
            ...
            <div className="w-full h-3 bg-gray-950 border border-gray-900 rounded-full overflow-hidden">
              <div className="h-full bg-cyan-500 transition-all duration-500" style={{ width: `${Math.max(0, (playerHp / (100 + playerStats.hp)) * 100)}%` }} />
            </div>
          </div>
        ...
        <div className="glass-panel p-4 border-red-500/20 bg-black/60 relative flex flex-col justify-between min-h-[160px]">
          <div className="flex items-center gap-3 md:flex-row-reverse md:text-right">
            <div className="relative transform scale-x-[-1]">
              <RenderAvatarPreview baseAvatar={opponent?.avatar || '👾'} outfit="none" expression="none" accessory="none" mount="none" size="md" />
            </div>
            <div>
              ...
              <div className="flex items-center gap-1.5 text-xs text-gray-400 font-mono mt-1 md:flex-row-reverse">
                <span>ROUND CARD:</span>
                {opponentSelectedCardId ? <span className="text-white font-bold">❓ 페이스다운</span> : <span className="text-red-500 font-bold">미선택</span>}
              </div>
            </div>
          </div>
          <div className="mt-4">
            ...
            <div className="w-full h-3 bg-gray-950 border border-gray-900 rounded-full overflow-hidden">
              <div className="h-full bg-red-500 transition-all duration-500" style={{ width: `${Math.max(0, (opponentHp / (100 + opponentStats.hp)) * 100)}%` }} />
            </div>
          </div>
              ...
              return (
                <button
                  ...
                  className={`p-3 border rounded-xl flex items-center justify-between btn-cyber transition-all ${
                    isUsed ? 'border-gray-950 bg-gray-950/20 opacity-30 cursor-not-allowed'
                    : isSelected ? 'bg-cyan-950/40 border-cyan-400 shadow-[0_0_8px_rgba(6,182,212,0.2)]'
                ...
                >
                  ...
                  <span className="text-[10px] font-mono text-cyan-400 font-bold">LV.{cardLvl} · ATK {card.power}</span>
                </button>
              );
            })}
          </div>
        ...
        <div className="lg:col-span-2 glass-panel p-6 border-cyan-500/15 bg-black/40 min-h-[300px] flex flex-col justify-between">
          ...
          {roundPhase === 'card_select' && (
            <div className="flex flex-col items-center justify-center h-full space-y-6 py-4">
              ...
              <div className="text-4xl font-black text-white font-mono-numbers">{roundTimer}s</div>
              ...
              <div className="w-full max-w-md p-3 border border-cyan-500/10 bg-cyan-950/5 rounded-xl flex flex-col gap-2 mt-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    { key: 'revive', name: '기력의조각', emoji: '💊', desc: '+50 HP (HP 30 이하만)' },
                  ].map(item => {
                    const count = items?.[item.key as keyof ItemInventory] || 0;
                    return (
                      <button key={item.key} disabled={count <= 0}
                        ...
                        className={`p-2 border rounded-lg flex items-center justify-between text-left transition-all ${
                          count > 0 ? 'bg-gray-900/60 border-cyan-500/20 hover:border-cyan-400 hover:scale-102 active:scale-98'
                      ...
                      >
                        ...
                        <span className="text-[9px] font-mono font-bold text-cyan-400 bg-cyan-950/30 px-1 py-0.5 rounded shrink-0">{count}개</span>
                      </button>
                    );
                  })}
                </div>
          ...
          {roundPhase === 'quiz' && quizQuestion && (
            <div className="space-y-6">
              <div className="flex justify-between items-center border-b border-gray-900 pb-3">
                <span className="text-xs font-mono text-cyan-400 font-bold">💡 정답 시 데미지 버프 +50% 적용!</span>
                <div className="flex items-center gap-1 text-yellow-500 font-mono text-sm">
                  <Timer className="w-4 h-4" />
                  <span>{roundTimer}초 남음</span>
                </div>
              </div>
              <h3 className="text-lg font-bold leading-relaxed text-gray-100">{quizQuestion.question}</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {(quizQuestion as MCQuestion).options.map((option, idx) => {
                  ...
                  if (isQuizAnswered) {
                    if (isSelected) {
                      btnStyle = isCorrect ? 'border-green-500 bg-green-950/30 text-green-400 font-black' : 'border-red-500 bg-red-950/30 text-red-400';
                    } else if (isCorrect) {
                  ...
                  return (
                    <button key={idx} disabled={isQuizAnswered} onClick={() => onQuizAnswerSubmit(idx)}
              ...
              {(() => {
                ...
                return (
                  <div className="flex flex-col items-center gap-2 mt-2 max-w-md w-full p-3 bg-gray-950/60 border border-gray-900 rounded-xl">
                    <div className="text-xs font-bold text-cyan-400 tracking-wide">⚔️ 속성 상성 전투 결과</div>
                    <div className="grid grid-cols-2 gap-4 w-full text-[11px] font-mono mt-1 pt-1.5 border-t border-gray-900 text-left">
                      <div>
                        <span className="text-gray-500 block">내 공격 ({ATTRIBUTE_EMOJIS[getCardAttribute(pCard.unitId)]} → {ATTRIBUTE_EMOJIS[getCardAttribute(oCard.unitId)]})</span>
                      ...
                      <div>
                        <span className="text-gray-500 block">상대 공격 ({ATTRIBUTE_EMOJIS[getCardAttribute(oCard.unitId)]} → {ATTRIBUTE_EMOJIS[getCardAttribute(pCard.unitId)]})</span>
