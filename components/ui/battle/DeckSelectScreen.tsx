...
import { BattleMode } from '../../../lib/battle-engine';
import { BattleCard } from './battle-card';

interface DeckSelectScreenProps {
  battleMode: BattleMode;
  onSetBattleMode: (mode: BattleMode) => void;
...
export default function DeckSelectScreen({
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap justify-between items-center gap-3 border-b border-gray-900 pb-4">
        <div>
        ...
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-gray-500 font-mono uppercase tracking-widest">배틀 모드:</span>
          <button
            data-testid="mode-standard"
            onClick={() => onSetBattleMode('standard')}
          ...
          <button
            data-testid="mode-bestof3"
            onClick={() => onSetBattleMode('bestof3')}
            className={`px-3 py-1.5 text-xs font-bold rounded-lg border transition-all ${
              battleMode === 'bestof3'
                ? 'bg-amber-950/30 border-amber-400 text-amber-400'
                : 'bg-gray-950 border-gray-800 text-gray-500 hover:border-gray-700'
            }`}
          >
            베스트오브3 (2선승)
      ...
      {!hasEnoughCards ? (
        <div className="p-12 glass-panel border-red-500/20 bg-red-950/5 text-center space-y-4">
          ...
          <p className="text-sm text-gray-400 max-w-sm mx-auto leading-relaxed">
            출전하려면 3장 이상의 카드가 해금되어 있어야 합니다.
          </p>
          <button onClick={onBack} className="px-6 py-2.5 bg-gray-900 border border-gray-800 hover:border-gray-700 text-xs font-black rounded-lg">
            로비로 돌아가기
          </button>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 glass-panel p-6 border-cyan-500/10 flex flex-col justify-between">
            <div>
              ...
              {battleMode === 'bestof3' && (
                <div className="mb-4 p-2 bg-amber-950/20 border border-amber-500/20 rounded-lg text-[10px] text-amber-400 font-mono">
                  🏆 베스트오브3: 2라운드 먼저 이기는 쪽 승리 (최대 3라운드)
                </div>
              )}
